...
function D=dipole_kernelKS(varargin)
...
if strcmp(domain,'kspace')
    ...
    D(matrix_size(1)/2 + 1,matrix_size(2)/2 + 1,matrix_size(3)/2 + 1) = -2.0/3.0; 
    % The value of the filter at the centre of K-space should be -2/3
    % See previous work and graph on effect of value at centre of k-space.
    D(isnan(D)) = 0; % Can keep this to tidy any unexpected NaNs although there should not be any
    D = fftshift(D);
