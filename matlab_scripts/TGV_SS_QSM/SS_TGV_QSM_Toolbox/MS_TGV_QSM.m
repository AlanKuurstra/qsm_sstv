function out = MS_TGV_QSM(params)
    ...
    Et1 = conj(E1);     Et2 = conj(E2);     Et3 = conj(E3);     Dt = conj(D);
    ...
    DtD_mu1_E_sos    = Dt.*D + mu1*(E1tE1 + E2tE2 + E3tE3);
    ...
    a1 = DtD_mu1_E_sos; a2 = mu1I_mu0_E_wsos1;  a3 = mu1I_mu0_E_wsos2;  a4 = mu1I_mu0_E_wsos3;
    ...
    s1_1 = zeros(N); z1_1 = zeros(N);
    s1_2 = zeros(N); z1_2 = zeros(N);
    s1_3 = zeros(N); z1_3 = zeros(N);
    
    % Allocate memory for symmetrized gradient
    s0_1 = zeros(N); z0_1 = zeros(N); 
    s0_2 = zeros(N); z0_2 = zeros(N);
    s0_3 = zeros(N); z0_3 = zeros(N); 
    s0_4 = zeros(N); z0_4 = zeros(N);
    s0_5 = zeros(N); z0_5 = zeros(N);
    s0_6 = zeros(N); z0_6 = zeros(N);
    ...
    for outer = 1:maxOuterIter
        ...
        rhs1    = DtFy                 + mu1*(Et1.*F_z1_minus_s1_1 + Et2.*F_z1_minus_s1_2 + Et3.*F_z1_minus_s1_3);
...
%         disp(['Iter: ', num2str(outer), '   Update: ', num2str(x_update)])
        ...
        E_v2 = ifftn(E2.*Fv2);
        E_v3 = ifftn(E3.*Fv3);
        E_v4 = ifftn(E1.*Fv2 + E2.*Fv1)/2;
        E_v5 = ifftn(E1.*Fv3 + E3.*Fv1)/2;
        E_v6 = ifftn(E2.*Fv3 + E3.*Fv2)/2;
        ...
        % Update z1: Grad
        z1_1 = max(abs(Dx1-v1+s1_1)-alpha1/mu1,0).*sign(Dx1-v1+s1_1);
        z1_2 = max(abs(Dx2-v2+s1_2)-alpha1/mu1,0).*sign(Dx2-v2+s1_2);
        z1_3 = max(abs(Dx3-v3+s1_3)-alpha1/mu1,0).*sign(Dx3-v3+s1_3);
